...
if (!API_KEY || !SEARCH_ENGINE_ID) {
  throw new Error(
    "Missing GOOGLE_API_KEY or GOOGLESEARCH_ENGINE_ID from .env file. Please check your .env configuration.",
  );
}
...
async function searchAndScrape(searchQuery) {
  try {
    ...
    console.log(
      `[DEBUG] Google search returned ${response.data.items?.length || 0} items`,
    );
    ...
    for (const link of candidateLinks) {
      ...
      const isError =
        result.paragraph.startsWith("Error: Could not access") ||
