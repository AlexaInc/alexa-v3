...
class ProxyHelper {
  constructor() {
    ...
    const userBypass = (process.env.NO_PROXY || "")
      .split(",")
      .map((h) => h.trim())
      .filter(Boolean);
    ...
    console.log(
      `ℹ️ Proxy System: rejectUnauthorized=${this.rejectUnauthorized}, timeout=${this.timeout}`,
    );
  ...
  _createAgent() {
    ...
    const isSupported = supportedProtocols.some((p) =>
      finalProxyUrl.startsWith(p),
    );
    const isLocalSidecar = finalProxyUrl.includes("127.0.0.1:10808");

    if (isSupported && !isLocalSidecar) {
      console.log(
        `🚀 Proxy System: V2Ray protocol detected. Routing through local Xray sidecar (127.0.0.1:10808)...`,
      );
    ...
    if (finalProxyUrl.startsWith("socks")) {
      console.log(
        `🚀 Proxy System: Initializing SOCKS Proxy Agent for ${finalProxyUrl.split("@").pop()}`,
      );
      return new SocksProxyAgent(finalProxyUrl, options);
    } else {
      console.log(
        `🚀 Proxy System: Initializing HTTPS Proxy Agent for ${finalProxyUrl.split("@").pop()}`,
      );
  ...
  configureGlobal() {
    if (!this.agent || this.disableGlobal) {
      if (this.disableGlobal)
        console.log(
          "ℹ️ Global Node.js Proxy override is DISABLED via PROXY_DISABLE_GLOBAL",
        );
      return;
    }
  ...
  async checkProxyConnectivity() {
    ...
    try {
      ...
      if (!tcpOk) {
        console.error(
          "❌ Proxy is UNREACHABLE (TCP failed). HF might be blocking this IP/Port.",
        );
        return false;
      }
      ...
      try {
        ...
        console.log(
          "ℹ️ Bot will still attempt connection, but failure is likely.",
        );
