  ...
  for (const chatId in gameSessions) {
    ...
    if (game && game.state === "playing" && game.lastActivityTime) {
      ...
      if (timeSinceLastActivity > INACTIVITY_TIMEOUT_MS) {
        console.log(
          `Hangman: Ending game in chat ${chatId} due to inactivity.`,
        );
...
try {
  ...
  console.log(
    `Hangman: Successfully loaded ${wordList.length} words from ${FILE_PATH}`,
  );
} catch (err) {
  console.error(`Hangman: Error reading ${FILE_PATH}:`, err.message);
  console.error(
    "Please make sure words.txt exists and contains one word per line.",
  );
...
async function handleHangmanGuess(msg, AlexaInc, guess) {
  ...
  if (game.allGuessedLetters.includes(guess)) {
    return AlexaInc.sendMessage(
      chatId,
      { text: `The letter \`${guess}\` has already been guessed.` },
      { quoted: msg },
    );
  }
  ...
  if (game.secretWord.includes(guess)) {
    ...
    const currentDisplay = getWordDisplay(
      game.secretWord,
      game.allGuessedLetters,
    );

    // Check for WIN
    if (!currentDisplay.includes("_")) {
      let winMessage =
        `🎉 *${username} guessed the last letter!* 🎉\n\n` +
        `The word was: \`\`\`${game.secretWord}\`\`\`\n\n` +
        `*--- 🏆 WINNERS 🏆 ---*\n`;
    ...
    const allPlayersOut = Object.values(game.players).every(
      (p) => p.wrongGuesses >= MAX_WRONG_GUESSES,
    );
...
async function handleNewHang(msg, AlexaInc) {
  ...
  if (!chatId.endsWith("@g.us")) {
    return AlexaInc.sendMessage(
      chatId,
      {
        text: "This game mode is for groups only. Add me to a group to start a game.",
      },
      { quoted: msg },
    );
  }
  if (gameSessions[chatId]) {
    return AlexaInc.sendMessage(
      chatId,
      {
        text: "A game is already running. Type /joinhang to join it, or /endhang to stop it.",
      },
      { quoted: msg },
    );
  }
...
async function handleJoinHang(msg, AlexaInc) {
  ...
  if (!game) {
    return AlexaInc.sendMessage(
      chatId,
      { text: "No game is active. Start one with /newhang." },
      { quoted: msg },
    );
  }
  if (game.state !== "joining") {
    return AlexaIns.sendMessage(
      chatId,
      { text: "This game is already in progress. Wait for the next one!" },
      { quoted: msg },
    );
  }
  ...
  if (Object.keys(game.players).length >= MAX_PLAYERS) {
    return AlexaInc.sendMessage(
      chatId,
      { text: `The game is full! (Max ${MAX_PLAYERS} players).` },
      { quoted: msg },
    );
  }
...
async function handleStartHang(msg, AlexaInc) {
  ...
  if (!game) {
    return AlexaInc.sendMessage(
      chatId,
      { text: "No game to start. Start one with /newhang." },
      { quoted: msg },
    );
  }
  ...
  if (Object.keys(game.players).length === 0) {
    return AlexaInc.sendMessage(
      chatId,
      {
        text: "Cannot start the game with no players. Type /joinhang to play.",
      },
      { quoted: msg },
    );
  }
...
async function handleEndHang(msg, AlexaInc) {
  ...
  if (gameSessions[chatId]) {
    ...
    return AlexaInc.sendMessage(
      chatId,
      { text: "Game stopped. Start a new one with /newhang." },
      { quoted: msg },
    );
  } else {
    return AlexaInc.sendMessage(
...
async function handleHangLead(msg, AlexaInc) {
  ...
  if (allPlayerData.length === 0) {
    return AlexaInc.sendMessage(
      chatId,
      { text: "The leaderboard is empty. Be the first to win!" },
      { quoted: msg },
    );
  }
  ...
  const mentions = []; // <-- Array to hold the JIDs
