...
let restartHistory = [];
// NOTE: app.js writes restart history to <repo root>/data/restarts.json
// (see restartLogFile in app.js). This MUST read from the exact same path —
// it previously read "./restarts.json" (missing the "data/" folder AND
// depending on process.cwd() at spawn time), so restartHistory here was
// always empty and the "Your bot Alexa is ready!" WhatsApp message always
// fell back to "Restart id: N/A ... Reason: Startup".
const RESTART_LOG_FILE = path.join(__dirname, "..", "data", "restarts.json");
try {
  if (fs.existsSync(RESTART_LOG_FILE)) {
    restartHistory = JSON.parse(fs.readFileSync(RESTART_LOG_FILE, "utf8"));
  }
} catch (e) {
