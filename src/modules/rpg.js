...
async function ensureUser(userId) {
  ...
  const [rows] = await db
    .promise()
    .query("SELECT * FROM rpg_users WHERE user_id = ?", [userId]);
  if (rows.length === 0) {
    await db
      .promise()
      .query("INSERT IGNORE INTO rpg_users (user_id) VALUES (?)", [userId]);
    return {
      user_id: userId,
      class: null,
      level: 1,
...
async function chooseClass(userId, className) {
  ...
  if (u.class) {
    return {
      ok: false,
      message: `❌ You're already a *${u.class}*. Classes can't be changed (for now).`,
    };
  }
  const match = CLASSES.find((c) =>
    c.toLowerCase().startsWith((className || "").toLowerCase()),
  );
  if (!match) {
    return {
      ok: false,
      message: `❌ Invalid class. Choose one:\n${CLASSES.map((c) => `• ${c}`).join("\n")}\n\nUsage: .class <name>`,
    };
  }

  await db
    .promise()
    .query("UPDATE rpg_users SET class = ? WHERE user_id = ?", [match, userId]);
  return {
    ok: true,
    message: `🎉 You awakened as a *${match}*! Use *.train* to grow your power.`,
  };
}
...
async function train(userId) {
  ...
  if (wait > 0) {
    return {
      ok: false,
      message: `⏳ You're exhausted from training. Rest for ${Math.ceil(wait / 60000)} more minute(s).`,
    };
  }
...
async function dungeon(userId) {
  ...
  if (wait > 0) {
    return {
      ok: false,
      message: `⏳ The dungeon gate is sealed. Try again in ${Math.ceil(wait / 60000)} minute(s).`,
    };
  }
...
async function leaderboard(limit = 10) {
  ...
  if (rows.length === 0)
    return {
      message: "📊 No RPG data yet. Use .class then .train to start!",
      mentions: [],
    };
