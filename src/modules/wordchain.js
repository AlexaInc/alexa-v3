...
try {
  if (fs.existsSync(DICTIONARY_FILE)) {
    ...
    MAX_POSSIBLE_LENGTH = chainDictionary.reduce(
...
async function eliminatePlayer(chatId, userId, AlexaInc) {
  ...
  const username = game.playerNames[userId];
  await AlexaInc.sendMessage(chatId, {
    text: `💀 *Time's up!* @${userId.split("@")[0]} eliminated!`,
    mentions: [userId],
  });
...
async function handleStartChain(msg, AlexaInc) {
  ...
  if (chainSessions[chatId]) {
    return AlexaInc.sendMessage(
      chatId,
      { text: "A Word Chain game is already active in this chat." },
      { quoted: msg },
    );
  }
  if (chainDictionary.length === 0) {
    return AlexaInc.sendMessage(
      chatId,
      { text: "⚠️ Dictionary not loaded. Cannot start game." },
      { quoted: msg },
    );
  }
  ...
  const joinInterval = setInterval(async () => {
    ...
    if (timeLeftSeconds > 0) {
      await AlexaInc.sendMessage(chatId, {
        text: `⏳ *${timeLeftSeconds} seconds* left to join!`,
      });
    } else {
      // TIME IS UP
      clearInterval(joinInterval);
      ...
      if (game.players.length >= 2) {
        ...
        await AlexaInc.sendMessage(chatId, {
          text: "❌ Not enough players (Min 2). Game cancelled.",
        });
        delete chainSessions[chatId];
      }
    }
...
async function handleJoinChain(msg, AlexaInc) {
  ...
  if (game && game.state === "playing") {
    return AlexaInc.sendMessage(
      chatId,
      { text: `game already startet wait for next game` },
      { quoted: msg },
    );
  } else {
    return AlexaInc.sendMessage(
      chatId,
      { text: `there are no games started to join firsh use \`/startchain\`` },
      { quoted: msg },
    );
  }
}
...
async function handleStopChain(msg, AlexaInc) {
  ...
  if (chainSessions[chatId]) {
    ...
    return AlexaInc.sendMessage(
      chatId,
      { text: "🛑 Word Chain game stopped manually." },
      { quoted: msg },
    );
  }
}
...
async function handleChainGuess(msg, AlexaInc, text) {
  ...
  if (text.length < game.currentMinWordLength) {
    return AlexaInc.sendMessage(
      chatId,
      { text: `⚠️ Too short! Need ${game.currentMinWordLength}+ letters.` },
      { quoted: msg },
    );
  }
  if (text.charAt(0) !== game.lastLetter) {
    return AlexaInc.sendMessage(
      chatId,
      { text: `❌ Must start with *${game.lastLetter.toUpperCase()}*` },
      { quoted: msg },
    );
  }
...
function checkInactiveChainGames(AlexaInc) {
  ...
  for (const id in chainSessions) {
    if (now - chainSessions[id].lastActivityTime > INACTIVITY_MS) {
      if (chainSessions[id].turnTimer)
        clearTimeout(chainSessions[id].turnTimer);
      delete chainSessions[id];
      AlexaInc.sendMessage(id, {
        text: "💤 Word Chain game closed due to inactivity.",
      });
    }
  }
}
