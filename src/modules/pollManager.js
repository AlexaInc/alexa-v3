...
async function sendPollAndSendResult(
  ...
  try {
    if (!mainStore) {
      console.error(
        "[PollManager] Error: Poll listener not initialized. Call initPollListener() first.",
      );
      return;
    }

    console.log(`[PollManager] Sending poll to ${jid}...`);
    const sentPoll = await socket.sendMessage(jid, {
      poll: {
        name: pollName,
        values: pollOptions,
        selectableCount: 1,
    ...
    console.log(
      `[PollManager] Poll ${pollKey} is now active for ${timeoutInSeconds}s.`,
    );
