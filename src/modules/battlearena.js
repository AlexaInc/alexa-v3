...
function playerMove(chatId, user, move) {
  ...
  const opponent = game.p1 === user ? game.p2 : game.p1;
  let log = "";
  ...
  if (move === "attack") {
    ...
    if (game.cooldown[user] > 0) {
      return {
        error: true,
        message: `⏳ Special on cooldown (${game.cooldown[user]} turns left)`,
      };
    }
...
🎖 +1 win added to leaderboard`,
      players: [
        game.p1.replace(/^@?(\d+)$/, "$1@lid"),
        game.p2.replace(/^@?(\d+)$/, "$1@lid"),
      ],
    };
  }
