/*
 * this file is part of alexainc/alexa-v3
 * owned by hansaka
 *
 * ---------------------------------------------------------------------------
 * memoryStats.js — accurate, `free -h` compatible memory reporting.
 * ---------------------------------------------------------------------------
 *
 * WHY THIS FILE EXISTS
 * --------------------
 * Both the control panel (/sysstats) and the WhatsApp `.menu` command used to
 * report RAM as:
 *
 *     used = total - free            // systeminformation's mem.used / os module
 *
 * On Linux that number is meaningless for "how busy is my box?", because the
 * kernel deliberately fills every spare byte with page cache. A healthy 1 GB
 * VM therefore reported 90-95% memory usage while `free -h` showed it was
 * barely a third full:
 *
 *     $ free -h
 ...
 *
 *     total - free      = 887Mi  ->  93%   <-- what the dashboard showed (wrong)
 *     free's "used"     = 355Mi  ->  37%   <-- what it should show
 *
 * This module reads /proc/meminfo directly and reproduces procps' arithmetic
 * exactly, so the panel and `free -h` always agree:
 *
 *     buff/cache = Buffers + Cached + SReclaimable
 *     used       = MemTotal - MemFree - buff/cache
 *     available  = MemAvailable          (what a new process can actually get)
 *
 * It is also container aware. Inside Docker / Hugging Face Spaces /proc/meminfo
 * still shows the *host's* RAM, which makes the gauge lie in the other
 * direction. When a cgroup memory limit is present we report the limit instead,
 * because that is the number the OOM killer actually cares about.
 *
 * No external dependency: plain fs + os, so it works in every process
 * (app.js, index.js, server.js) without loading systeminformation.
 */
