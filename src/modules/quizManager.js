...
// Configuration
const QUIZ_URL =
  "https://raw.githubusercontent.com/hansaka02/questionjson/main/quiz.json";
...
const globalLeaderboard = new Map(); // Global scores across all groups
...
async function sendNextQuestion(AlexaInc, jid) {
  ...
  const buttons = questionData.options.map((option, index) => {
    ...
    const dmPayload = encodeURIComponent(
      `${QUIZ_MAGIC_PREFIX}${encryptedPayload}`,
    );
...
function handleDMAnswer(AlexaInc, jid, text) {
  ...
  for (const [groupJid, quiz] of activeQuizzes.entries()) {
    if (quiz.sessionId === sessionId) {
      if (!quiz.answers.has(jid)) {
        quiz.answers.set(jid, answerCode);
        AlexaInc.sendMessage(jid, {
          text: `✅ Answer *${answerCode}* recorded for this session.`,
        });
      } else {
        AlexaInc.sendMessage(jid, {
  ...
  AlexaInc.sendMessage(jid, {
    text: `Sorry, this question session is no longer active.`,
  });
  return;
}
...
async function startQuiz(AlexaInc, jid) {
  if (activeQuizzes.has(jid)) {
    return AlexaInc.sendMessage(jid, {
      text: "⚠️ A quiz is already running in this group.",
    });
  }
...
async function stopQuiz(AlexaInc, jid) {
  const currentQuiz = activeQuizzes.get(jid);
  if (!currentQuiz)
    return AlexaInc.sendMessage(jid, { text: "⚠️ No active quiz." });
...
async function sendFinalLeaderboard(AlexaInc, jid) {
  ...
  const sorted = Array.from(globalLeaderboard.entries()).sort(
    (a, b) => b[1] - a[1],
  );
