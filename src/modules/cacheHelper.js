...
async function getCachedGroupMetadata(arg1, arg2) {
  ...
  if (!jid) {
    console.error(
      "[CacheHelper] getCachedGroupMetadata called with undefined JID",
    );
    return null;
  }
  ...
  if (!socket) {
    console.warn(
      `[CacheHelper] No socket available for metadata fetch of ${jid}`,
    );
    return null;
  }
  ...
  const requestPromise = (async () => {
    try {
      ...
      console.error(
        `[CacheHelper] Error fetching group metadata for ${jid}:`,
        err.message,
      );
      return null;
    } finally {
...
async function getCachedGroupSettings(db, jid) {
  ...
  const requestPromise = (async () => {
    const fetchSettings = async (retry = true) => {
      try {
        ...
        if (
          ...
          console.warn(
            `[CacheHelper] DB connection lost for ${jid}, retrying...`,
          );
    ...
    try {
      ...
      console.error(
        `[CacheHelper] Error fetching group settings for ${jid}:`,
        err.message,
      );
      return null;
    } finally {
