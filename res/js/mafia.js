...
const Mafia = {
    ...
    async createGame(AlexaInc, msg, botNumber) {
        ...
        await AlexaInc.sendMessage(msg.key.remoteJid, {
            ...
            interactiveButtons: [{
                name: "cta_url",
                buttonParamsJson: JSON.stringify({ display_text: "Join Game", url: joinUrl })
            }]
        });
    },
    ...
    async handleNightAction(AlexaInc, msg, id, lid) {
        ...
        if (id === "_set_det_check" || id === "_set_det_kill") {
            if (game.nightActions['detective']) {
                return safeSend(AlexaInc, lid, { text: "⚠️ You have already chosen your action tonight." });
            }
            p.detectiveMode = (id === "_set_det_check") ? 'check' : 'kill';
            await safeSend(AlexaInc, lid, { text: `Mode set to ${p.detectiveMode.toUpperCase()}.` });
        ...
        if (id.startsWith("_n_")) {
            ...
            if (p.role === "don" || p.role === "mafia") {
                if (game.nightActions['mafia']) {
                    return safeSend(AlexaInc, lid, { text: "⚠️ The Mafia has already selected a target." });
                }
                game.nightActions['mafia'] = targetId;
            } else {
                if (game.nightActions[p.role]) {
                    return safeSend(AlexaInc, lid, { text: "⚠️ You have already performed your action." });
                }
                if (p.role === "detective") {
                    game.nightActions['detective'] = { type: p.detectiveMode, target: targetId };
                } else {
    ...
    async handleVote(AlexaInc, msg, id, lid) {
        ...
        if (id.startsWith("_day_vote_")) {
            if (game.votes[lid]) {
                return safeSend(AlexaInc, lid, { text: "⚠️ You have already voted! You cannot change it." });
            }

            const targetId = id.replace("_day_vote_", "");
            game.votes[lid] = targetId;

            await safeSend(AlexaInc, lid, { text: `✅ You voted for ${game.players[targetId].name}` });
