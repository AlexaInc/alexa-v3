...
function generateConfig() {
  ...
  if (proxyUrl.startsWith("vless://")) {
    try {
      ...
      const vlessMatch = proxyUrl.match(
        /^vless:\/\/([^@]+)@([^:]+):(\d+)(.*)$/i,
      );
      ...
      console.log(
        `ℹ️ Parsing VLESS for sidecar: ${hostname}:${port} (network=${params.type || "tcp"})`,
      );
    ...
    console.error(
      "❌ Unsupported or unknown proxy protocol. Sidecar might not start correctly.",
    );
  }
